...
app.post('/api/login', async (req, res) => {
    try {
        ...
        const result = await sql.query`
            SELECT u.id_usuario, u.username, u.email, u.password_hash, u.foto, u.id_rol, u.id_empleado,
        ...
        if (result.recordset.length > 0) {
            ...
            try {
                ...
                else if (userAgent.includes('Android')) { sistemaOperativo = 'Android'; dispositivo = 'Móvil'; }
                else if (userAgent.includes('iPhone') || userAgent.includes('iPad')) { sistemaOperativo = 'iOS'; dispositivo = 'Móvil'; }
                
                await sql.query`
                    INSERT INTO auditoria_login (id_usuario, fecha_inicio, direccion_ip, dispositivo, navegador, sistema_operativo, exitoso)
                    VALUES (${user.id_usuario}, GETDATE(), ${ip}, ${dispositivo}, ${navegador}, ${sistemaOperativo}, 1)
                `;
            } catch(e) {
                console.error('⚠️ Error registrando auditoría:', e.message);
            }
...
app.post('/api/logout', async (req, res) => {
    try {
        ...
        if (id_usuario) {
            try {
                await sql.query`
                    UPDATE TOP (1) auditoria_login 
                    SET fecha_cierre = GETDATE() 
                    WHERE id_usuario = ${id_usuario} AND fecha_cierre IS NULL 
                    ORDER BY fecha_inicio DESC
                `;
            } catch(e) {
                console.error('⚠️ Error actualizando auditoría:', e.message);
            }
        }
...
// ========== AUDITORÍA DE LOGIN ==========
app.get('/api/auditoria-login/:idUsuario', async (req, res) => {
    try {
...
app.post('/api/cambiar-password', async (req, res) => {
    try {
        ...
        await sql.query`UPDATE usuario SET password_hash = ${passwordHash} WHERE id_usuario = ${result.recordset[0].id_usuario}`;
...
app.get('/api/perfil-completo/:id', async (req, res) => {
    try {
        ...
        try { 
            const tels = await sql.query`SELECT telefono FROM empleado_telefono WHERE id_empleado = ${id}`; 
            empleado.telefonos = tels.recordset.map(t => t.telefono); 
        } catch(e) { console.error('Error teléfonos:', e.message); }
        
        try { 
            const correos = await sql.query`SELECT correo FROM empleado_correo WHERE id_empleado = ${id}`; 
            empleado.correos_extra = correos.recordset.map(c => c.correo); 
        } catch(e) { console.error('Error correos:', e.message); }
        
        try { 
            const dirs = await sql.query`
                SELECT ed.*, d.nombre as departamento, m.nombre as municipio 
                FROM empleado_direccion ed 
                LEFT JOIN departamento d ON ed.id_departamento = d.id_departamento 
                LEFT JOIN municipio m ON ed.id_municipio = m.id_municipio 
                WHERE ed.id_empleado = ${id}
            `; 
            empleado.direcciones = dirs.recordset; 
        } catch(e) { console.error('Error direcciones:', e.message); }
        
        try { 
            const alergias = await sql.query`
                SELECT ea.id_alergia, a.nombre as alergia, ea.observacion 
                FROM empleado_alergia ea 
                INNER JOIN alergia a ON ea.id_alergia = a.id_alergia 
                WHERE ea.id_empleado = ${id}
            `; 
            empleado.alergias = alergias.recordset; 
        } catch(e) { console.error('Error alergias:', e.message); }
        
        try { 
            const estudios = await sql.query`
                SELECT ee.id_estudio, ee.id_nivel_academico, ee.institucion, ee.titulo_obtenido, ee.fecha_finalizacion, 
                       na.nombre as nivel_academico 
                FROM empleado_estudio ee 
                LEFT JOIN nivel_academico na ON ee.id_nivel_academico = na.id_nivel_academico 
                WHERE ee.id_empleado = ${id} 
                ORDER BY ee.fecha_finalizacion DESC
            `; 
            empleado.estudios = estudios.recordset; 
        } catch(e) { console.error('Error estudios:', e.message); }
        
        try { 
            const docs = await sql.query`
                SELECT ed.id_documento, ed.nombre_archivo, ed.ruta_archivo, ed.fecha_carga, ed.observacion, 
                       td.nombre as tipo_documento 
                FROM empleado_documento ed 
                LEFT JOIN tipo_documento_empleado td ON ed.id_tipo_documento = td.id_tipo_documento 
                WHERE ed.id_empleado = ${id} AND ed.activo = 1 
                ORDER BY ed.fecha_carga DESC
            `; 
            empleado.documentos = docs.recordset; 
        } catch(e) { console.error('Error documentos:', e.message); }
...
app.post('/api/usuarios', async (req, res) => {
    try {
        ...
        await sql.query`INSERT INTO usuario (username, email, password_hash, id_rol, id_empleado) VALUES (${username}, ${email}, ${passwordHash}, ${parseInt(id_rol)}, ${id_empleado})`;
...
app.put('/api/usuarios/:id', async (req, res) => {
    try {
        ...
        if (password) {
            const passwordHash = await bcrypt.hash(password, saltRounds);
            await sql.query`UPDATE usuario SET username=${username}, email=${email}, password_hash=${passwordHash}, id_rol=${id_rol}, activo=${activo} WHERE id_usuario=${id}`;
        } else {
...
app.delete('/api/usuarios/:id/foto', async (req, res) => {
    try {
        const { id } = req.params;
        const u = await sql.query`SELECT foto FROM usuario WHERE id_usuario = ${id}`;
        const anterior = u.recordset[0]?.foto;
        if (anterior) {
            const ruta = path.join(__dirname, 'public', 'uploads', anterior);
            if (fs.existsSync(ruta)) fs.unlinkSync(ruta);
            await sql.query`UPDATE usuario SET foto = NULL WHERE id_usuario = ${id}`;
        }
        res.json({ success: true });
    } catch (error) { res.status(500).json({ success: false, message: error.message }); }
});
...
// ========== CATEGORÍAS Y TIPOS DE INSUMO ==========
app.get('/api/categorias-producto', async (req, res) => {
    try { const r = await sql.query`SELECT id_categoria_producto, nombre FROM categoria_producto WHERE activo = 1 ORDER BY nombre`; res.json(r.recordset); } 
    catch (error) { res.status(500).json([]); }
});

app.get('/api/tipos-insumo', async (req, res) => {
    try { const r = await sql.query`SELECT id_tipo_insumo, nombre FROM tipo_insumo WHERE activo = 1 ORDER BY nombre`; res.json(r.recordset); } 
    catch (error) { res.status(500).json([]); }
});
...
// ========== INVENTARIOS CON VERIFICACIÓN DE ESTADO ==========
app.get('/api/inventario-plata', async (req, res) => {
    try { 
        const r = await sql.query`
            SELECT ip.id_plata as id, ip.codigo_unico as codigo, ip.descripcion as nombre, 
                   ip.peso, ip.precio_costo, ip.precio_publico as precio, 
                   CASE 
                       WHEN dv.id_detalle_venta IS NOT NULL THEN 'Vendido'
                       ELSE 'Disponible'
                   END as estado,
                   cp.nombre as categoria 
            FROM inventario_plata ip 
            INNER JOIN categoria_producto cp ON ip.id_categoria_producto = cp.id_categoria_producto 
            LEFT JOIN detalle_venta dv ON dv.tipo_producto = 'plata' AND dv.id_producto = ip.id_plata
            WHERE ip.activo = 1 
            ORDER BY ip.fecha_registro DESC
        `; 
        res.json(r.recordset); 
    } catch (error) { 
        console.error('Error inventario plata:', error);
...
app.get('/api/inventario-oro', async (req, res) => {
    try { 
        const r = await sql.query`
            SELECT io.id_oro as id, io.codigo_unico as codigo, io.descripcion as nombre, 
                   io.peso, io.precio_costo, io.precio_publico as precio, 
                   CASE 
                       WHEN dv.id_detalle_venta IS NOT NULL THEN 'Vendido'
                       ELSE 'Disponible'
                   END as estado,
            ...
            INNER JOIN categoria_producto cp ON io.id_categoria_producto = cp.id_categoria_producto 
            LEFT JOIN detalle_venta dv ON dv.tipo_producto = 'oro' AND dv.id_producto = io.id_oro
            WHERE io.activo = 1 
            ORDER BY io.fecha_registro DESC
        `; 
        res.json(r.recordset); 
    } catch (error) { 
        console.error('Error inventario oro:', error);
        res.status(500).json({ message: error.message }); 
    }
});
...
app.get('/api/inventario-acero', async (req, res) => {
    try { 
        const r = await sql.query`
            SELECT ia.id_acero as id, ia.codigo_unico as codigo, ia.descripcion as nombre, 
                   ia.precio_costo, ia.precio_publico as precio, 
                   CASE 
                       WHEN dv.id_detalle_venta IS NOT NULL THEN 'Vendido'
                       ELSE 'Disponible'
                   END as estado,
                   cp.nombre as categoria 
            FROM inventario_acero ia 
            INNER JOIN categoria_producto cp ON ia.id_categoria_producto = cp.id_categoria_producto 
            LEFT JOIN detalle_venta dv ON dv.tipo_producto = 'acero' AND dv.id_producto = ia.id_acero
            WHERE ia.activo = 1 
            ORDER BY ia.fecha_registro DESC
        `; 
        res.json(r.recordset); 
    } catch (error) { 
        console.error('Error inventario acero:', error);
        res.status(500).json({ message: error.message }); 
    }
});
...
app.get('/api/inventario-empaques', async (req, res) => {
    try { 
        const r = await sql.query`
            SELECT ie.id_empaque as id, ie.codigo_unico as codigo, ie.descripcion as nombre, 
                   ie.precio_costo, ie.precio_publico as precio, 
                   CASE 
                       WHEN dv.id_detalle_venta IS NOT NULL THEN 'Vendido'
                       ELSE 'Disponible'
                   END as estado
            FROM inventario_empaque ie 
            LEFT JOIN detalle_venta dv ON dv.tipo_producto = 'empaque' AND dv.id_producto = ie.id_empaque
            WHERE ie.activo = 1 
            ORDER BY ie.fecha_registro DESC
        `; 
        res.json(r.recordset); 
    } catch (error) { 
        console.error('Error inventario empaques:', error);
        res.status(500).json({ message: error.message }); 
    }
});
...
app.get('/api/inventario-insumos', async (req, res) => {
    try { 
        const r = await sql.query`
            SELECT ii.id_insumo as id, ii.codigo, ii.descripcion as nombre, 
                   ti.nombre as tipo, ii.cantidad, ii.precio_costo, ii.precio_publico as precio,
                   CASE 
                       WHEN ii.cantidad <= 0 THEN 'Agotado'
                       ELSE 'Disponible'
                   END as estado
            FROM inventario_insumo ii 
            INNER JOIN tipo_insumo ti ON ii.id_tipo_insumo = ti.id_tipo_insumo 
            WHERE ii.activo = 1 
            ORDER BY ii.fecha_registro DESC
        `; 
        res.json(r.recordset); 
    } catch (error) { 
        console.error('Error inventario insumos:', error);
        res.status(500).json({ message: error.message }); 
    }
});
...
app.post('/api/ingreso-producto', async (req, res) => {
    try {
        ...
        switch (tipo) {
            case 'plata': await sql.query`INSERT INTO inventario_plata (codigo_unico, descripcion, peso, precio_costo, precio_publico, id_categoria_producto, id_estado_producto, id_ingreso) VALUES (${codigo}, ${descripcion}, ${peso || 0}, ${precio_costo}, ${precio_publico}, ${id_categoria || 1}, 1, ${id_ingreso})`; break;
            case 'oro': await sql.query`INSERT INTO inventario_oro (codigo_unico, descripcion, peso, precio_costo, precio_publico, id_categoria_producto, id_estado_producto, id_ingreso) VALUES (${codigo}, ${descripcion}, ${peso || 0}, ${precio_costo}, ${precio_publico}, ${id_categoria || 1}, 1, ${id_ingreso})`; break;
            case 'acero': await sql.query`INSERT INTO inventario_acero (codigo_unico, descripcion, precio_costo, precio_publico, id_categoria_producto, id_estado_producto, id_ingreso) VALUES (${codigo}, ${descripcion}, ${precio_costo}, ${precio_publico}, ${id_categoria || 1}, 1, ${id_ingreso})`; break;
            case 'empaque': await sql.query`INSERT INTO inventario_empaque (codigo_unico, descripcion, precio_costo, precio_publico, id_estado_producto, id_ingreso) VALUES (${codigo}, ${descripcion}, ${precio_costo}, ${precio_publico}, 1, ${id_ingreso})`; break;
...
app.get('/api/ingresos', async (req, res) => {
    try {
        const r = await sql.query`
            SELECT i.numero_ingreso, i.fecha, i.observacion, i.id_usuario,
...
app.post('/api/ventas', async (req, res) => {
    try {
        const { numero_factura, id_cliente, id_empleado, id_usuario, id_sucursal, id_estado_venta, subtotal, descuento, total, id_metodo_pago, productos } = req.body;
        
        const result = await sql.query`
            INSERT INTO venta (numero_factura, id_cliente, id_empleado, id_usuario, id_sucursal, id_estado_venta, subtotal, descuento, total) 
            OUTPUT INSERTED.id_venta 
            VALUES (${numero_factura}, ${id_cliente}, ${id_empleado || 1}, ${id_usuario || 1}, ${id_sucursal || 1}, ${id_estado_venta || 1}, ${subtotal}, ${descuento}, ${total})
        `;
        ...
        if (productos && Array.isArray(productos)) {
            for (const prod of productos) {
                try {
                    await sql.query`
                        INSERT INTO detalle_venta (id_venta, tipo_producto, id_producto, descripcion, cantidad, precio_unitario) 
                        VALUES (${id_venta}, ${prod.tipo}, ${prod.id}, ${prod.nombre}, 1, ${prod.precio})
                    `;
                } catch(e) {
                    console.error('Error insertando detalle:', e.message);
                }
            }
        }
        ...
        if (id_metodo_pago) {
            try {
                await sql.query`
                    ...
                    VALUES (${id_venta}, ${id_metodo_pago}, ${total})
                `;
            } catch(e) {
                console.error('Error insertando pago:', e.message);
            }
        }
...
app.get('/api/ventas', async (req, res) => {
    try {
        const result = await sql.query`
            SELECT v.id_venta, v.numero_factura, v.fecha, v.subtotal, v.descuento, v.total,
                   c.primer_nombre, c.segundo_nombre, c.primer_apellido, c.segundo_apellido, c.nit,
                   e.primer_nombre as nombre_vendedor, e.primer_apellido as apellido_vendedor,
        ...
        const ventas = result.recordset.map(v => ({
            ...
            fecha: v.fecha,
            cliente: [v.primer_nombre, v.segundo_nombre, v.primer_apellido, v.segundo_apellido].filter(Boolean).join(' '),
            nit: v.nit,
            vendedor: [v.nombre_vendedor, v.apellido_vendedor].filter(Boolean).join(' '),
            subtotal: v.subtotal,
...
app.get('/api/venta-detalle/:id', async (req, res) => {
    try {
        const { id } = req.params;
        
        const detalle = await sql.query`
            SELECT dv.descripcion as nombre, dv.precio_unitario as precio, dv.cantidad
            FROM detalle_venta dv
            WHERE dv.id_venta = ${id}
        `;
        
        res.json({ success: true, productos: detalle.recordset });
    } catch (error) {
        console.error('Error obteniendo detalle:', error);
        res.status(500).json({ success: false, productos: [] });
    }
});
...
// ========== ESTADOS DE PRODUCTO ==========
app.get('/api/estados-producto', async (req, res) => {
    try { const r = await sql.query`SELECT * FROM estado_producto ORDER BY id_estado_producto`; res.json(r.recordset); } 
    catch (error) { res.status(500).json([]); }
});

// ========== MÉTODOS DE PAGO ==========
app.get('/api/metodos-pago', async (req, res) => {
    try { const r = await sql.query`SELECT * FROM metodo_pago WHERE activo = 1 ORDER BY nombre`; res.json(r.recordset); } 
    catch (error) { res.status(500).json([]); }
});

// ========== ESTADOS DE ORDEN ==========
app.get('/api/estados-orden', async (req, res) => {
    try { const r = await sql.query`SELECT * FROM estado_orden WHERE activo = 1 ORDER BY nombre`; res.json(r.recordset); } 
    catch (error) { res.status(500).json([]); }
});

// ========== TIPOS DE REPARACIÓN ==========
app.get('/api/tipos-reparacion', async (req, res) => {
    try { const r = await sql.query`SELECT * FROM tipo_reparacion WHERE activo = 1 ORDER BY nombre`; res.json(r.recordset); } 
    catch (error) { res.status(500).json([]); }
});

// ========== CATÁLOGOS DE FABRICACIÓN ==========
app.get('/api/catalogos-fabricacion', async (req, res) => {
    try {
        const codigos = await sql.query`SELECT * FROM codigo_anillo WHERE activo = 1 ORDER BY codigo`;
        const tallas = await sql.query`SELECT * FROM talla_anillo ORDER BY codigo`;
        res.json({ codigos: codigos.recordset, tallas: tallas.recordset });
    } catch (error) { res.status(500).json({ codigos: [], tallas: [] }); }
});

// ========== MOTIVOS DE AJUSTE ==========
app.get('/api/motivos-ajuste', async (req, res) => {
    try { 
        const r = await sql.query`SELECT id_motivo_ajuste, nombre, descripcion FROM motivo_ajuste WHERE activo = 1 ORDER BY nombre`; 
        res.json(r.recordset); 
    } catch (error) { 
        res.status(500).json([]); 
    }
...
// ========== PRODUCTOS PARA AJUSTE ==========
app.get('/api/productos-ajuste/:tipo', async (req, res) => {
    try {
        const { tipo } = req.params;
        let query = '';
        switch(tipo) {
            case 'plata':
                query = `SELECT ip.id_plata as id, ip.codigo_unico, ip.descripcion as nombre, ip.peso, 
                                ip.precio_costo, ip.precio_publico, ip.id_categoria_producto, ip.id_estado_producto,
                                ep.nombre as estado, cp.nombre as categoria
                         FROM inventario_plata ip
                         INNER JOIN estado_producto ep ON ip.id_estado_producto = ep.id_estado_producto
                         INNER JOIN categoria_producto cp ON ip.id_categoria_producto = cp.id_categoria_producto
                         WHERE ip.activo = 1`;
                break;
            case 'oro':
                query = `SELECT io.id_oro as id, io.codigo_unico, io.descripcion as nombre, io.peso, 
                                io.precio_costo, io.precio_publico, io.id_categoria_producto, io.id_estado_producto,
                                ep.nombre as estado, cp.nombre as categoria
                         FROM inventario_oro io
                         INNER JOIN estado_producto ep ON io.id_estado_producto = ep.id_estado_producto
                         INNER JOIN categoria_producto cp ON io.id_categoria_producto = cp.id_categoria_producto
                         WHERE io.activo = 1`;
                break;
            case 'acero':
                query = `SELECT ia.id_acero as id, ia.codigo_unico, ia.descripcion as nombre, 
                                ia.precio_costo, ia.precio_publico, ia.id_categoria_producto, ia.id_estado_producto,
                                ep.nombre as estado, cp.nombre as categoria
                         FROM inventario_acero ia
                         INNER JOIN estado_producto ep ON ia.id_estado_producto = ep.id_estado_producto
                         INNER JOIN categoria_producto cp ON ia.id_categoria_producto = cp.id_categoria_producto
                         WHERE ia.activo = 1`;
                break;
            case 'empaque':
                query = `SELECT ie.id_empaque as id, ie.codigo_unico, ie.descripcion as nombre, 
                                ie.precio_costo, ie.precio_publico, ie.id_estado_producto,
                                ep.nombre as estado
                         FROM inventario_empaque ie
                         INNER JOIN estado_producto ep ON ie.id_estado_producto = ep.id_estado_producto
                         WHERE ie.activo = 1`;
                break;
            case 'insumo':
                query = `SELECT ii.id_insumo as id, ii.codigo, ii.descripcion as nombre, 
                                ii.id_tipo_insumo, ii.cantidad, ii.precio_costo, ii.precio_publico,
                                ti.nombre as tipo
                         FROM inventario_insumo ii
                         INNER JOIN tipo_insumo ti ON ii.id_tipo_insumo = ti.id_tipo_insumo
                         WHERE ii.activo = 1`;
        ...
        console.error('Error productos ajuste:', error);
        res.status(500).json([]);
    }
});
...
app.get('/api/ajustes', async (req, res) => {
    try {
        const r = await sql.query`
            SELECT a.id_ajuste, a.fecha, a.observacion,
                   u.username as usuario,
                   m.nombre as motivo,
                   (SELECT COUNT(*) FROM detalle_ajuste da WHERE da.id_ajuste = a.id_ajuste) as total_productos
            FROM ajuste_inventario a
            INNER JOIN usuario u ON a.id_usuario = u.id_usuario
            INNER JOIN motivo_ajuste m ON a.id_motivo_ajuste = m.id_motivo_ajuste
            ORDER BY a.fecha DESC
        `;
        res.json(r.recordset);
    } catch (error) {
        console.error('Error historial ajustes:', error);
        res.status(500).json([]);
    }
});

// ========== APLICAR AJUSTE ==========
app.post('/api/ajustes/aplicar', async (req, res) => {
    try {
        const { tipo, id_motivo_ajuste, id_usuario, observacion, productos } = req.body;
        
        const ajusteResult = await sql.query`
            INSERT INTO ajuste_inventario (id_usuario, id_motivo_ajuste, observacion)
            OUTPUT INSERTED.id_ajuste
            VALUES (${id_usuario || 1}, ${id_motivo_ajuste}, ${observacion || null})
        `;
        const id_ajuste = ajusteResult.recordset[0].id_ajuste;
        
        for (const prod of productos) {
            const { id, descripcion, precio_costo, precio_publico, precio_anterior, cantidad_anterior, cantidad_nueva } = prod;
            
            switch(tipo) {
                case 'plata':
                    await sql.query`UPDATE inventario_plata SET descripcion=${descripcion}, precio_costo=${precio_costo}, precio_publico=${precio_publico}, peso=${prod.peso || 0}, id_categoria_producto=${prod.id_categoria_producto || 1}, id_estado_producto=${prod.id_estado_producto || 1} WHERE id_plata=${id}`;
                    await sql.query`INSERT INTO detalle_ajuste (id_ajuste, tabla_producto, id_producto, cantidad_anterior, cantidad_nueva, diferencia) VALUES (${id_ajuste}, 'plata', ${id}, 1, 1, 0)`;
                    break;
                case 'oro':
                    await sql.query`UPDATE inventario_oro SET descripcion=${descripcion}, precio_costo=${precio_costo}, precio_publico=${precio_publico}, peso=${prod.peso || 0}, id_categoria_producto=${prod.id_categoria_producto || 1}, id_estado_producto=${prod.id_estado_producto || 1} WHERE id_oro=${id}`;
                    await sql.query`INSERT INTO detalle_ajuste (id_ajuste, tabla_producto, id_producto, cantidad_anterior, cantidad_nueva, diferencia) VALUES (${id_ajuste}, 'oro', ${id}, 1, 1, 0)`;
                    break;
                case 'acero':
                    await sql.query`UPDATE inventario_acero SET descripcion=${descripcion}, precio_costo=${precio_costo}, precio_publico=${precio_publico}, id_categoria_producto=${prod.id_categoria_producto || 1}, id_estado_producto=${prod.id_estado_producto || 1} WHERE id_acero=${id}`;
                    await sql.query`INSERT INTO detalle_ajuste (id_ajuste, tabla_producto, id_producto, cantidad_anterior, cantidad_nueva, diferencia) VALUES (${id_ajuste}, 'acero', ${id}, 1, 1, 0)`;
                    break;
                case 'empaque':
                    await sql.query`UPDATE inventario_empaque SET descripcion=${descripcion}, precio_costo=${precio_costo}, precio_publico=${precio_publico}, id_estado_producto=${prod.id_estado_producto || 1} WHERE id_empaque=${id}`;
                    await sql.query`INSERT INTO detalle_ajuste (id_ajuste, tabla_producto, id_producto, cantidad_anterior, cantidad_nueva, diferencia) VALUES (${id_ajuste}, 'empaque', ${id}, 1, 1, 0)`;
                    break;
                case 'insumo':
                    await sql.query`UPDATE inventario_insumo SET descripcion=${descripcion}, precio_costo=${precio_costo}, precio_publico=${precio_publico}, id_tipo_insumo=${prod.id_tipo_insumo || 1}, cantidad=${cantidad_nueva} WHERE id_insumo=${id}`;
                    const diferencia = cantidad_nueva - cantidad_anterior;
                    await sql.query`INSERT INTO detalle_ajuste (id_ajuste, tabla_producto, id_producto, cantidad_anterior, cantidad_nueva, diferencia) VALUES (${id_ajuste}, 'insumo', ${id}, ${cantidad_anterior}, ${cantidad_nueva}, ${diferencia})`;
                    break;
            }
        }
        
        res.json({ success: true, message: 'Ajuste aplicado correctamente', id_ajuste });
    } catch (error) {
...
// ========== TIPOS DE INVENTARIO PARA AJUSTES ==========
app.get('/api/tipos-inventario-ajuste', async (req, res) => {
    try {
        const tipos = [];
        
        const checkPlata = await sql.query`SELECT COUNT(*) as total FROM inventario_plata WHERE activo = 1`;
        if (checkPlata.recordset[0].total > 0) tipos.push({ valor: 'plata', nombre: 'Plata', icono: 'fa-ring' });
        
        const checkOro = await sql.query`SELECT COUNT(*) as total FROM inventario_oro WHERE activo = 1`;
        if (checkOro.recordset[0].total > 0) tipos.push({ valor: 'oro', nombre: 'Oro', icono: 'fa-crown' });
        
        const checkAcero = await sql.query`SELECT COUNT(*) as total FROM inventario_acero WHERE activo = 1`;
        if (checkAcero.recordset[0].total > 0) tipos.push({ valor: 'acero', nombre: 'Acero', icono: 'fa-gem' });
        
        const checkEmpaque = await sql.query`SELECT COUNT(*) as total FROM inventario_empaque WHERE activo = 1`;
        if (checkEmpaque.recordset[0].total > 0) tipos.push({ valor: 'empaque', nombre: 'Empaque', icono: 'fa-box-open' });
        
        const checkInsumo = await sql.query`SELECT COUNT(*) as total FROM inventario_insumo WHERE activo = 1`;
        if (checkInsumo.recordset[0].total > 0) tipos.push({ valor: 'insumo', nombre: 'Insumo', icono: 'fa-tools' });
        
        res.json(tipos);
    } catch (error) {
        console.error('Error tipos inventario:', error);
        res.status(500).json([]);
    }
});

// ========== MANTENIMIENTO DE CATÁLOGOS (CORREGIDO - sin sql.request) ==========
const tablasPermitidasMantenimiento = [
    'departamento', 'municipio', 'genero', 'nacionalidad', 'estado_civil',
    'tipo_sangre', 'profesion', 'cargo_empleado', 'tipo_contrato',
    'nivel_academico', 'alergia', 'rol', 'permiso', 'tipo_documento_empleado',
    'categoria_producto', 'estado_producto', 'tipo_insumo', 'tipo_movimiento',
    'estado_ingreso', 'tipo_servicio', 'metodo_pago', 'estado_venta',
    'estado_orden', 'tipo_reparacion', 'codigo_anillo', 'talla_anillo',
    'color_piedra', 'material_anillo', 'motivo_ajuste'
];

const pkMapMantenimiento = {
    departamento: 'id_departamento', municipio: 'id_municipio', genero: 'id_genero',
    nacionalidad: 'id_nacionalidad', estado_civil: 'id_estado_civil', tipo_sangre: 'id_tipo_sangre',
    profesion: 'id_profesion', cargo_empleado: 'id_cargo', tipo_contrato: 'id_tipo_contrato',
    nivel_academico: 'id_nivel_academico', alergia: 'id_alergia', rol: 'id_rol',
    permiso: 'id_permiso', tipo_documento_empleado: 'id_tipo_documento',
    categoria_producto: 'id_categoria_producto', estado_producto: 'id_estado_producto',
    tipo_insumo: 'id_tipo_insumo', tipo_movimiento: 'id_tipo_movimiento',
    estado_ingreso: 'id_estado_ingreso', tipo_servicio: 'id_tipo_servicio',
    metodo_pago: 'id_metodo_pago', estado_venta: 'id_estado_venta',
    estado_orden: 'id_estado_orden', tipo_reparacion: 'id_tipo_reparacion',
    codigo_anillo: 'id_codigo_anillo', talla_anillo: 'id_talla_anillo',
    color_piedra: 'id_color_piedra', material_anillo: 'id_material_anillo',
    motivo_ajuste: 'id_motivo_ajuste'
};

app.get('/api/mantenimiento/:tabla', async (req, res) => {
    try {
        const { tabla } = req.params;
        
        if (!tablasPermitidasMantenimiento.includes(tabla)) {
            return res.status(400).json({ success: false, message: 'Tabla no permitida' });
        }
        
        const r = await sql.query(`SELECT * FROM ${tabla} ORDER BY 1 DESC`);
        res.json({ success: true, data: r.recordset });
    } catch (error) {
        console.error('Error en mantenimiento GET:', error);
        res.status(500).json({ success: false, message: error.message });
    }
});

app.post('/api/mantenimiento/:tabla', async (req, res) => {
    try {
        const { tabla } = req.params;
        ...
        const columnas = Object.keys(datos).filter(k => datos[k] !== null && datos[k] !== '');
        ...
        const valoresPlaceholder = columnas.map(k => {
            const valor = datos[k];
            if (typeof valor === 'number' || (!isNaN(valor) && valor.trim() !== '' && /^-?\d+(\.\d+)?$/.test(valor))) {
                return valor;
            }
            return `'${String(valor).replace(/'/g, "''")}'`;
        }).join(', ');
        
        const query = `INSERT INTO ${tabla} (${nombres}) VALUES (${valoresPlaceholder})`;
        ...
        console.error('Error en mantenimiento POST:', error);
        res.status(500).json({ success: false, message: error.message });
    }
});

app.put('/api/mantenimiento/:tabla/:id', async (req, res) => {
    try {
        ...
        const pk = pkMapMantenimiento[tabla];
        if (!pk) return res.status(400).json({ success: false, message: 'PK no encontrada' });
        ...
        const sets = columnas.map(k => {
            const valor = datos[k];
            if (typeof valor === 'number' || (!isNaN(valor) && valor.trim() !== '' && /^-?\d+(\.\d+)?$/.test(valor))) {
                return `${k} = ${valor}`;
            }
            return `${k} = '${String(valor).replace(/'/g, "''")}'`;
        }).join(', ');
        
        const query = `UPDATE ${tabla} SET ${sets} WHERE ${pk} = ${id}`;
        ...
        console.error('Error en mantenimiento PUT:', error);
        res.status(500).json({ success: false, message: error.message });
    }
});

app.delete('/api/mantenimiento/:tabla/:id', async (req, res) => {
    try {
        const { tabla, id } = req.params;
        ...
        await sql.query(`DELETE FROM ${tabla} WHERE ${pk} = ${id}`);
        
        res.json({ success: true, message: 'Registro eliminado' });
    } catch (error) {
        console.error('Error en mantenimiento DELETE:', error);
        res.status(500).json({ success: false, message: error.message });
    }
});
